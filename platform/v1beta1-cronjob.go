...
func execV1Beta1CronJob(k kubernetes.Interface, objdep *v1beta1.CronJob, namespace string, opts configuration.Options, commandFlag configuration.CommandFlag) (state.State, error) {
	logger.Info("Found CronJob resource")
	dsclient := k.BatchV1beta1().CronJobs(namespace)

	if opts.DryRun {
		_, err := dsclient.Get(objdep.Name, v12.GetOptions{})
		if err != nil {
			logger.Error(fmt.Sprintf("DRY-RUN: CronJob resource %s does not exist\n", objdep.Name))
			return state.EDeploymentStateNotExists, err
		} else {
			logger.Info(fmt.Sprintf("DRY-RUN: CronJob resource %s exists\n", objdep.Name))
	...
	if commandFlag == configuration.Replace {
		...
		if err != nil {
			logger.Error(fmt.Sprintf("Could not deploy CronJob resource %s due to %s", objdep.Name, err.Error()))
			return state.EDeploymentStateError, err
		}
		logger.Info("CronJob deployed")
	...
	if commandFlag == configuration.Apply {
		...
		if err != nil {
			logger.Error("Could not update CronJob")
