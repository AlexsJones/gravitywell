...
func execV1SecretResouce(k kubernetes.Interface, objdep *v1.Secret, namespace string, opts configuration.Options, commandFlag configuration.CommandFlag) (state.State, error) {
	logger.Info("Found Secret resource")
	client := k.CoreV1().Secrets(namespace)

	if opts.DryRun {
		_, err := client.Get(objdep.Name, meta1.GetOptions{})
		if err != nil {
			logger.Error(fmt.Sprintf("DRY-RUN: Secret resource %s does not exist\n", objdep.Name))
			return state.EDeploymentStateNotExists, err
		}
		logger.Info(fmt.Sprintf("DRY-RUN: Secret resource %s exists\n", objdep.Name))
	...
	if commandFlag == configuration.Replace {
		...
		if err != nil {
			logger.Error(fmt.Sprintf("Could not deploy Secret resource %s due to %s", objdep.Name, err.Error()))
			return state.EDeploymentStateError, err
		}
		logger.Info("Secret deployed")
	...
	if commandFlag == configuration.Apply {
		...
		if err != nil {
			logger.Error("Could not update Secret")
