...
func execV1ConfigMapResouce(k kubernetes.Interface, objdep *v1.ConfigMap, namespace string, opts configuration.Options, commandFlag configuration.CommandFlag) (state.State, error) {
	logger.Info("Found Configmap resource")
	cmclient := k.CoreV1().ConfigMaps(namespace)

	if opts.DryRun {
		_, err := cmclient.Get(objdep.Name, v12.GetOptions{})
		if err != nil {
			logger.Error(fmt.Sprintf("DRY-RUN: Configmap resource %s does not exist\n", objdep.Name))
			return state.EDeploymentStateNotExists, err
		} else {
			logger.Info(fmt.Sprintf("DRY-RUN: Configmap resource %s exists\n", objdep.Name))
	...
	if commandFlag == configuration.Replace {
		...
		if err != nil {
			logger.Error(fmt.Sprintf("Could not deploy ConfigMap resource %s due to %s", objdep.Name, err.Error()))
	...
	if commandFlag == configuration.Create {
		...
		logger.Info("ConfigMap deployed")
		return state.EDeploymentStateOkay, nil
	}
	//Apply --------------------------------------------------------------------
	if commandFlag == configuration.Apply {
		...
		if err != nil {
			logger.Error("Could not update ConfigMap")
