...
func execV1Pod(k kubernetes.Interface, objdep *v1.Pod, namespace string, opts configuration.Options, commandFlag configuration.CommandFlag) (state.State, error) {
	logger.Info("Found Pod resource")
	client := k.CoreV1().Pods(namespace)

	if opts.DryRun {
		_, err := client.Get(objdep.Name, v12.GetOptions{})
		if err != nil {
			logger.Error(fmt.Sprintf("DRY-RUN: Pod resource %s does not exist\n", objdep.Name))
			return state.EDeploymentStateNotExists, err
		} else {
			logger.Info(fmt.Sprintf("DRY-RUN: Pod resource %s exists\n", objdep.Name))
	...
	if commandFlag == configuration.Replace {
		...
		if err != nil {
			logger.Error(fmt.Sprintf("Could not deploy Pod resource %s due to %s", objdep.Name, err.Error()))
