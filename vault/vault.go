...
func UnSetVaultConfiguration(opt configuration.Options, cluster configuration.ProviderCluster) error {
	if cluster.Vault.Path != "" {
		if cluster.Vault.Url != "" {
			...
			defer resp.Body.Close()
			color.Green(fmt.Sprintf("Cluster %s unset on Vault Cluster", cluster.Name))
		} else {
			...
			err = vcs.Commit(gvcs, vaultTempDir, cluster.Vault.Repo.Url, opt.SSHKeyPath, fmt.Sprintf("Remove kubernetes secret setting for %s on %s project", cluster.Name, cluster.Project))
			if err != nil {
				return err
			}
			...
			color.Green(fmt.Sprintf("Cluster %s unset on git repo %s on branch %s on the folder %s/unset.", cluster.Name, cluster.Vault.Repo.Url, cluster.Vault.Repo.Branch, cluster.Vault.Repo.Path))
		}
	}
	return nil
}

func SetVaultConfiguration(opt configuration.Options, cluster configuration.ProviderCluster) error {
	...
	if cluster.Vault.Path != "" {
		...
		if cluster.Vault.Url != "" {
			...
			defer resp.Body.Close()
			color.Green(fmt.Sprintf("Cluster %s set on Vault Cluster with path: %s", cluster.Name, cluster.Vault.Path))
		} else {
			var extension = filepath.Ext(cluster.Vault.Repo.Url)
			var remoteVCSRepoName = cluster.Vault.Repo.Url[0 : len(cluster.Vault.Repo.Url)-len(extension)]
			splitStrings := strings.Split(remoteVCSRepoName, "/")
			...
			err = ioutil.WriteFile(vaultConfigPath, vaultAuthJsonContent, 0644)
			if err != nil {
				return err
			}
			...
			err = vcs.Commit(gvcs, vaultTempDir, cluster.Vault.Repo.Url, opt.SSHKeyPath, fmt.Sprintf("Set kubernetes secret setting for %s on %s project", cluster.Name, cluster.Project))
			if err != nil {
				return err
			}
			...
			defer os.RemoveAll(vaultTempDir)
			color.Green(fmt.Sprintf("Cluster %s set on git repo %s on branch %s on the folder %s/set.", cluster.Name, cluster.Vault.Repo.Url, cluster.Vault.Repo.Branch, cluster.Vault.Repo.Path))
		}
	}
