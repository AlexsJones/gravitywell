...
type IVCS interface {
	Fetch(localpath string, remote string, keypath string) (string, error)
	Update(localpath string) (string, error)
}

//Fetch from a remote VCS
func Fetch(v IVCS, localpath string, remote string, keypath string) (string, error) {
	return v.Fetch(localpath, remote, keypath)
}
